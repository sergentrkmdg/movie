...
const Navbar = () => {
    ...
    return (
    <div className='flex items-center  justify-between  bg-neutral-900  md:px-5 px-1 h-28'>
        <div onClick={()=>navigate(`/`) } className='lg:text-4xl text-2xl text-transparent bg-gradient-to-r bg-clip-text   from-green-500 to-sky-400  font-bold tracking-wider '> SINEFILM</div>
        <div className='text-white md:space-x-2 space-x-1 font-medium divide-x divide divide-slate-300  divide-opacity-50'>
        ...
        <div className='flex items-center md:space-x-4 space-x-1'>  
            <input value={search} onKeyPress={searchPost} onChange={e=>setSearch(e.target.value)}  className='hover:border md:w-48 w-28  hover:opacity-100 text-white opacity-70 bg-neutral-700 p-2 outline-none rounded-xl ' type="text" placeholder='Search movie' />
